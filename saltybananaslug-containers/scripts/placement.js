...
function makePlacementHud(name) {
  ...
  hud.className = "sbs-placement-hud";
  hud.innerHTML = `<strong><i class="fas fa-crosshairs"></i> Place ${foundry.utils.escapeHTML(name || "Container")}</strong><span>Move over the scene and left-click to place. Right-click or press Esc to cancel.</span>`;
 ...
 */
export async function chooseContainerPlacement({name="Container", image=""}={}) {
  if (!canvas?.ready || !canvas?.scene || !canvas?.stage) throw new Error("Open an active scene before placing a container.");
  ...
  return new Promise(resolve => {
    ...
    const cancel = () => {
      cleanup();
      ui.notifications.info("Container placement cancelled.");
      resolve(null);
    };
