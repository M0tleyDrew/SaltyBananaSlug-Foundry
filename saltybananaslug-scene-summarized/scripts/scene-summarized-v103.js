...
function getSceneCounts(scene) {
  ...
  const totalItems = tokens.reduce((sum, token) => sum + (token.actor?.items?.size ?? token.actor?.items?.length ?? 0), 0);
  return {
...
function buildOverviewPage(scene, options, counts) {
  ...
  const gmInventory = `
    ...
    ${table([
      ...
      ["Embedded Actor Items", formatNumber(counts.items)],
...
function buildCastPage(scene, options) {
  ...
  const cards = tokens.map((token, index) => {
    ...
    const publicSubtitle = safe ? "Visible token" : (actor ? escapeHTML(actorTypeText(actor).replace(/<[^>]*>/g, "")) : "No Actor attached");
  ...
  return `<section class="sbs-scene-summary-journal"><h1>Cast & Creatures</h1><p>Every placed token, its Actor, statistics, effects, vision, emitted light, and technical token configuration.</p>${cards}</section>`;
}
...
function buildItemsPage(scene, options) {
  ...
  return `<section class="sbs-scene-summary-journal"><h1>Items, Loot & Equipment</h1><p>Physical inventory and equipment carried by each unique placed Actor. Actor features, monster actions, class features, and spells are kept with their Actor on the Cast & Creatures page.</p>${groups.join("") || "<p><em>No physical inventory or equipment was found on placed Actors.</em></p>"}</section>`;
}
...
function buildRawDataPage(scene, options) {
  ...
  const content = `<p>This is a complete serialized snapshot of the Scene document at generation time. It can be enormous. That is not a bug; it is the forensic goblin fulfilling its oath.</p><pre>${escapeHTML(safeJSON(source))}</pre>`;
...
class SceneSummarizedApplication extends Application {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      id: MODULE_ID,
      title: MODULE_TITLE,
      template: `${MODULE_PATH}/templates/scene-summarized-v103.html`,
      width: 960,
      height: 860,
      resizable: true,
  ...
  getData() {
    ...
    return {
      ...
      sceneCounts: counts ? `${counts.tokens} tokens · ${counts.items} Actor Items · ${counts.tiles} tiles · ${counts.sounds} sounds · ${counts.notes} notes · ${counts.walls} walls` : "",
